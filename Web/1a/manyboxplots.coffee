...
draw = (data) ->
  ...
  w = 1150
  h = 455
  pad = {left:90, top:40, right:60, bottom: 50}
  ...
  svg = d3.select("div#chart1").append("svg")
          .attr("width", w)
          .attr("height", h)
  ...
  rightAxis.selectAll("empty")
       ...
       .attr("y", (d,i) -> yScale((i+0.5)/nQuant*1.4 - 0.7))
  ...
  # lower svg
  lowsvg = d3.select("div#chart2").append("svg")
             .attr("height", h)
             .attr("width", w)
  ...
  svg.append("text")
     ...
     .attr("x", pad.left*0.12)
     .attr("y", h/2)
     .attr("fill", "blue")
     .attr("transform", "rotate(270, #{pad.left*0.12}, #{h/2})")
     .attr("dominant-baseline", "middle")
     .attr("text-anchor", "middle")
